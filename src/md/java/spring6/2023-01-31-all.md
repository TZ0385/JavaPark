...
控制反转，也就是之前文章中一直在提的 `IoC`，它不是一门技术，而是一种设计思想，是一个重要的面向对象编程法则。通过使用 `IoC`，我们能够设计出松耦合、更优秀的程序。

`Spring` 通过 `IoC` 来管理所有 `Java` 对象的实例化和初始化，控制对象之间的依赖关系。我们将由 `IoC` 容器管理的 `Java` 对象称之为 `SpringBean`，它跟用关键字 `new` 锁创建的 `Java` 对象没有任何区别。
