...
上一篇中，我们讲了 [如何搭建 Java 开发环境](https://cunyu1943.github.io/posts/9a989e71/)。不过既然确定了要从事 Java 开发，那怎么能少得了一个趁手的 IDE 呢。正所谓工欲善其事。必先利其器，一个趁手的 IDE 就像电视剧里的神兵利器，能让我们功力大增，大大提高我们的效率。
...
安装过程和 Ubuntu 大致相同，只是 Windows 中的安装包是可执行文件，只需要双击进行安装即可，然后后面选择自己的安装路径即可，安装好之后的设置和 Ubuntu 一致。具体的，可以查看我的另一篇详细介绍在 Windows 中安装并简单配置 IDEA 的文章：[IntelliJ IDEA 下载安装及配置教程](https://cunyu1943.github.io/posts/4e8ef0d4/)
...
前一篇文章中，我们搭建了 Java 开发环境的同时，介绍了如何利用编辑器写我们的 `Hello World`，接下来就讲讲如何利用 IDEA 来创建并书写我们的 `Hello World` 程序。
