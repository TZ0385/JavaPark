<template><div><h2 id="一个-java-文件中是否可以存在多个类-修饰类除外" tabindex="-1"><a class="header-anchor" href="#一个-java-文件中是否可以存在多个类-修饰类除外" aria-hidden="true">#</a> 一个 Java 文件中是否可以存在多个类（修饰类除外）？</h2>
<p>一个 Java 文件中是可以存在多个类的，但是一个 Java 文件中只能存在一个 <code v-pre>public</code> 所修饰的类，而且这个 Java 文件的文件名还必须和 <code v-pre>public</code> 所修饰类的类名保持一致，一个简单的实例如下。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="java-有哪些特点" tabindex="-1"><a class="header-anchor" href="#java-有哪些特点" aria-hidden="true">#</a> Java 有哪些特点？</h2>
<ol>
...
<li>C/C++ 中，字符串和字符数组最后均有一个额外的 <code v-pre>\0</code> 标志来表示结束，但 Java 中不存在这一概念。</li>
</ul>
</li>
</ul>
<h2 id="jre-和-jdk-有什么不同。" tabindex="-1"><a class="header-anchor" href="#jre-和-jdk-有什么不同。" aria-hidden="true">#</a> JRE  和 JDK 有什么不同。</h2>
<ul>
...
<h2 id="java-程序编译过程" tabindex="-1"><a class="header-anchor" href="#java-程序编译过程" aria-hidden="true">#</a> Java 程序编译过程</h2>
<p><img src="https://s1.ax1x.com/2020/07/25/UzFVeO.png" alt="" loading="lazy"></p>
<p>我们编译的源代码（<code v-pre>xxx.java</code>）经 JDK 中的 <code v-pre>javac</code> 命令编译后，成为 JVM 能够理解的 Java 字节码（<code v-pre>xxx.class</code>），然后经由 JVM 加载，通过解释器 <strong>逐行解释执行</strong>，这就是为什么能经常听见说 <strong>Java 是一门编译和解释共存的语言</strong>。</p>
<p>其中 JVM 是解释 Java 字节码（<code v-pre>xxx.class</code>） 的虚拟机，针对不同系统均有特定实现，方便一次编译，多次运行，即 Java 语言的平台独立性；</p>
<h2 id="说一下-java-中的数据类型" tabindex="-1"><a class="header-anchor" href="#说一下-java-中的数据类型" aria-hidden="true">#</a> 说一下 Java 中的数据类型</h2>
...
<h2 id="封装类" tabindex="-1"><a class="header-anchor" href="#封装类" aria-hidden="true">#</a> 封装类</h2>
...
<h2 id="缓存池" tabindex="-1"><a class="header-anchor" href="#缓存池" aria-hidden="true">#</a> 缓存池</h2>
<p>以 <code v-pre>new Integer(123)</code> 和 <code v-pre>Integer.valueOf(123)</code> 为例：</p>
<ul>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上述代码为 Java 8 中 String 的定义，其底层实际上使用的是字符（<code v-pre>char</code>）数组，而且由于被声明为 <code v-pre>final</code>，代表着它 <strong>不能被继承</strong>。而且一旦初始化之后就不能再去引用其他数组，这样就保证了 <code v-pre>String</code> 的不可变性，也因此 String 是线程安全的。</p>
<h3 id="不可变性的优点" tabindex="-1"><a class="header-anchor" href="#不可变性的优点" aria-hidden="true">#</a> 不可变性的优点</h3>
<ol>
...
<h3 id="string-vs-stringbuffer-vs-stringbuffer" tabindex="-1"><a class="header-anchor" href="#string-vs-stringbuffer-vs-stringbuffer" aria-hidden="true">#</a> String vs StringBuffer vs StringBuffer</h3>
<p>主要从三个方面对三者进行对比：</p>
<table>
<thead>
<tr>
...
<h3 id="字符串常量池-string-pool" tabindex="-1"><a class="header-anchor" href="#字符串常量池-string-pool" aria-hidden="true">#</a> 字符串常量池（String Pool）</h3>
<p>String Pool 位于 <strong>方法区</strong>，通常保存着所有 <a href="https://baike.baidu.com/item/%E5%AD%97%E9%9D%A2%E9%87%8F" target="_blank" rel="noopener noreferrer">字符串字面量（literal strings）<ExternalLinkIcon/></a>，在编译期间就被确定。此外，还可以用 <code v-pre>String</code> 中的 <code v-pre>intern()</code> 方法在运行过程中添加到 String Pool 中。当一个字符串调用 <code v-pre>intern()</code> 时，如果 String Pool 中已经存在字面量相同的字符串，则会返回 String Pool 中的引用；如果不存在，则向 String Pool 中添加一个新的字符串，同时返回新字符串的引用；</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token class-name">String</span> str1 <span class="token operator">=</span> <span class="token string">"村雨遥"</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://s1.ax1x.com/2020/07/25/UxLfq1.png" alt="" loading="lazy"></p>
<h3 id="new-string-xxx" tabindex="-1"><a class="header-anchor" href="#new-string-xxx" aria-hidden="true">#</a> new String(“xxx”)</h3>
...
* <span class="token keyword">@param</span> : name，公众号
*/</span>
<span class="token class-name">String</span> name <span class="token operator">=</span> <span class="token string">"村雨遥"</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="常见关键字" tabindex="-1"><a class="header-anchor" href="#常见关键字" aria-hidden="true">#</a> 常见关键字</h3>
<p><img src="https://cdn.jsdelivr.net/gh/cunyu1943/image-hosting-for-blog/imgJava 关键字.png" alt="Java 关键字" loading="lazy"></p>
<h3 id="标识符和关键字" tabindex="-1"><a class="header-anchor" href="#标识符和关键字" aria-hidden="true">#</a> 标识符和关键字</h3>
<ul>
...
<ol>
<li>标识符由英文字符大小写（a - z, A - Z）、数字（0 - 9）、下划线（<code v-pre>_</code>）和美元符号（<code v-pre>$</code>）组成；</li>
<li>不能以数字开头，不能是关键字；</li>
<li>严格区分大小写；</li>
<li>包名：多个单词组成是所有单词均小写；</li>
<li>类名和接口：大写驼峰命名法；</li>
<li>变量名和函数名：多个单词组成时，第一个单词全小写，其他单词采用大写驼峰命名法；</li>
<li>常量名：字母全部大写，单词之间用下划线（<code v-pre>_</code>）分割；</li>
</ol>
</li>
</ul>
<h2 id="说一下-java-中的访问控制" tabindex="-1"><a class="header-anchor" href="#说一下-java-中的访问控制" aria-hidden="true">#</a> 说一下 Java 中的访问控制</h2>
<p>Java 主要提供了 3 中访问修饰符 <code v-pre>public</code>、<code v-pre>protected</code>、<code v-pre>private</code>，但实际使用过程中，一共可以形成 4 种访问权限，分别是 <code v-pre>public</code>、<code v-pre>protected</code>、<code v-pre>private</code>、<code v-pre>default</code>，其中 <code v-pre>default</code> 是不加任何修饰符时的访问权限。关于 4 种访问权限的对比如下表，✅ 表示可以访问，❌ 表示不可访问。</p>
<table>
<thead>
<tr>
<th>作用域</th>
...
<h3 id="static、final、this、super" tabindex="-1"><a class="header-anchor" href="#static、final、this、super" aria-hidden="true">#</a> static、final、this、super</h3>
<ol>
<li><strong>static</strong></li>
</ol>
<p><code v-pre>static</code> 主要有如下 4 中使用场景：</p>
<ul>
<li><strong>修饰成员变量和成员方法</strong>：被 <code v-pre>static</code> 修饰的成员属于类，属于静态成员变量，存储在 Java 内存中的 <strong>方法区</strong>，不属于单个对象，被所有对象共享，而且最好通过 <code v-pre>类名.静态成员名/静态方法名()</code> 调用；</li>
<li><strong>静态代码块</strong>：定义在类中方法外，先于非静态代码块之前执行（<strong>静态代码块 -&gt; 非静态代码块 -&gt; 构造方法</strong>） ，而且不管执行多少次创建新对象的操作，静态代码只执行一次；</li>
<li><strong>静态内部类</strong>：<strong><code v-pre>static</code> 要修饰类时，只有修饰内部类这一种用法。</strong> 非静态内部类在编译后会隐含保存一个引用，用于指向创建它的外部类，但是静态内部类不存在。即 <strong>内部类的创建不用依赖外围类的创建，同时内部类也只能使用任意外部类的 <code v-pre>static</code> 成员变量和方法；</strong></li>
<li><strong>静态导包</strong>：用于导入静态资源，<code v-pre>import static</code> 用于指定导入某一类中的静态资源，然后我们就可以直接使用类中的静态成员变量和方法；</li>
<li><strong>注意：</strong>
<ul>
<li><code v-pre>abstract</code> 方法不能同时是 <code v-pre>static</code> 的，因为 <code v-pre>abstract</code> 方法需要被重写，但 <code v-pre>static</code> 方法不可以；</li>
<li>不能从 <code v-pre>static</code> 方法内部发出对非静态方法的调用，因为静态方法只能访问静态成员，而非静态方法的调用需要先创建对象；</li>
<li><code v-pre>static</code> 不能用于修饰局部变量；</li>
<li>内部类与静态内部类的区别：静态内部类相对外部类是独立存在的，在静态内部类中无法直接访问外部类中变量和方法。如果要进行访问，则必须 <code v-pre>new</code> 一个外部类对象，使用该对象来进行访问，但对于静态变量和静态方法，能够直接调用。而普通的内部类作为外部类的一个成员而存在，能够直接访问外部类属性，调用外部类方法。</li>
</ul>
</li>
</ul>
<ol start="2">
...
<ul>
<li>修饰类时，<strong>被修饰的类不能被继承，而且类中所有成员方法均被隐式指定为 <code v-pre>final</code> 方法</strong>；</li>
...
<p>用于引用类的当前实例，比如我们最常用的构造方法中，<strong>注意不能用在 <code v-pre>static</code> 方法中</strong>；</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">User</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其中 <code v-pre>this.age</code> 说明访问的是 <code v-pre>User</code> 类中的成员变量，而后面的 <code v-pre>age</code> 则代表传入的形参；</p>
<ol start="4">
<li><strong>super</strong></li>
</ol>
<p>用于从子类访问父类中的变量和方法，<strong>注意不能用在 <code v-pre>static</code> 方法中</strong>；</p>
...
<tbody>
<tr>
<td><code v-pre>continue</code></td>
<td>用于循环结构，指跳出当前循环，进入下一次循环</td>
</tr>
<tr>
<td><code v-pre>break</code></td>
<td>用于循环结构，指跳出整个循环体，继续执行循环下面的语句</td>
</tr>
<tr>
<td><code v-pre>return</code></td>
<td>1. <code v-pre>return ;</code> ：直接用 <code v-pre>return</code> 结束方法执行，用于没有返回值函数的方法；<br />2. <code v-pre>return value;</code> ：<code v-pre>return </code> 一个特定值，用于有返回值函数的方法</td>
</tr>
</tbody>
</table>
<h3 id="while-循环与-do-循环" tabindex="-1"><a class="header-anchor" href="#while-循环与-do-循环" aria-hidden="true">#</a> while 循环与 do 循环</h3>
<p><code v-pre>while</code> 循环结构在循环开始前会判断下一个迭代是否应该继续，可能一次循环体都不执行；</p>
<p><code v-pre>do……while</code> 会在循环的结果来判断是否继续下一轮迭代，<strong>至少会执行一次循环体</strong>；</p>
<h3 id="final、finally、finalize" tabindex="-1"><a class="header-anchor" href="#final、finally、finalize" aria-hidden="true">#</a> final、finally、finalize</h3>
<ol>
<li><strong>final</strong></li>
</ol>
<p><code v-pre>final</code> 既是一个修饰符，也是一个关键字，修饰不同对象时，表示的意义也不一样；</p>
<ul>
...
<li><strong>修饰方法</strong>：表示方法无法被重写，但是允许重载，<code v-pre>private</code> 方法会隐式指定为 <code v-pre>final</code> 方法；</li>
</ul>
<ol start="2">
<li><strong>finally</strong></li>
</ol>
<ul>
<li><code v-pre>finally</code> 是一个关键字，在异常处理时提供 <code v-pre>finally</code> 块来执行任何清除操作，无论是否有异常被抛出或捕获，<code v-pre>finally</code> 块均会被执行，通常用于释放资源；</li>
<li><code v-pre>finally</code> 正常情况下一定会被执行，但是在如下两种情况下不会执行：
<ul>
<li>对应的 <code v-pre>try</code> 未执行，则该 <code v-pre>try</code> 块的 <code v-pre>finally</code> 块并不会被执行；</li>
<li>若 <code v-pre>try</code> 块中 JVM 关机，则 <code v-pre>finally</code> 块也不会执行；</li>
</ul>
</li>
<li><code v-pre>finally</code> 中如果有 <code v-pre>return</code> 语句，则会覆盖 <code v-pre>try</code> 或 <code v-pre>catch</code> 中的 <code v-pre>return</code> 语句，导致两者无法 <code v-pre>return</code>，所以建议 <code v-pre>finally</code> 中不要存在 <code v-pre>return</code> 关键字；</li>
</ul>
<ol start="3">
<li><strong>finallize</strong></li>
</ol>
<p><code v-pre>finallize()</code> 是 <code v-pre>Object</code> 类的 <code v-pre>protected</code> 方法，子类能够覆盖该方法以实现资源清理工作；</p>
<p>GC 在回收前均会调用该方法，但是 <code v-pre>finalize()</code> 方法存在如下问题：</p>
<ul>
<li>Java 语言规范不保证 <code v-pre>finalize()</code> 方法会被及时执行，也不保证他们一定被执行；</li>
<li><code v-pre>finalize()</code> 方法会带来性能问题，因为 JVM 通常在单独的低优先线程中完成 <code v-pre>finalize</code> 的执行；</li>
<li><code v-pre>finalize()</code> 方法中，可将待回收对象赋值给 <code v-pre>GC Roots</code> 可达的对象引用，从而达到对象再生的目的；</li>
<li><code v-pre>finalize()</code> 方法最多由 GC 执行一次（但是可以手动调用对象的 <code v-pre>finalize</code> 方法）；</li>
</ul>
...
<td style="text-align:left"><code v-pre>（A &amp;&amp; B）</code>为假。</td>
</tr>
<tr>
...
<td style="text-align:left"><code v-pre>!(A &amp;&amp; B)</code>为真。</td>
</tr>
</tbody>
</table>
...
<h3 id="条件运算符" tabindex="-1"><a class="header-anchor" href="#条件运算符" aria-hidden="true">#</a> 条件运算符（? :）</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="instanceof" tabindex="-1"><a class="header-anchor" href="#instanceof" aria-hidden="true">#</a> instanceof</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="equals-和" tabindex="-1"><a class="header-anchor" href="#equals-和" aria-hidden="true">#</a> equals() 和 ==</h3>
<ul>
<li><code v-pre>==</code></li>
</ul>
<p>基本数据类型用 <code v-pre>==</code> 比较的是值，用于引用数据类型时判断两个对象的内存地址是否相等，即两对象是否是同一个对象；</p>
<blockquote>
...
<h3 id="方法的类型" tabindex="-1"><a class="header-anchor" href="#方法的类型" aria-hidden="true">#</a> 方法的类型</h3>
<ol>
<li>无参无返回值；</li>
<li>无参有返回值；</li>
<li>有参无返回值；</li>
...
<p>重写是当子类继承自父类的相同方法，输入数据一样，但最终响应不同于父类。重写发生在 <strong>运行期</strong>，是子类对父类允许访问的方法的实现逻辑进行改写。重写方法的方法名、参数列表以及返回值必须相同，抛出的异常范围不超出父类，访问修饰符的范围也不能小于父类。此外，若父类方法别 <code v-pre>private/final/static</code> 修饰，则子类无法重写父类方法，但 <code v-pre>static</code> 修饰的方法能被再次声明。<strong>构造方法是个特例，不能被重写</strong>。总结起来就是：<strong>重写即子类对父类方法的改造，外部样子不能改变，但能够改变内部逻辑</strong>。</p>
<ol start="3">
...
<h3 id="深-浅拷贝" tabindex="-1"><a class="header-anchor" href="#深-浅拷贝" aria-hidden="true">#</a> 深/浅拷贝</h3>
<ol>
...
<p>推荐阅读：<a href="https://juejin.im/post/5bce68226fb9a05ce46a0476" target="_blank" rel="noopener noreferrer">https://juejin.im/post/5bce68226fb9a05ce46a0476<ExternalLinkIcon/></a></p>
<h4 id="形参和实参" tabindex="-1"><a class="header-anchor" href="#形参和实参" aria-hidden="true">#</a> 形参和实参</h4>
<ol>
<li><strong>形参</strong>：方法被调用时需要传递进来的参数，如 <code v-pre>func(String name)</code> 中的 <code v-pre>name</code> 就是一个形参，只有在 <code v-pre>func</code> 被调用时 <code v-pre>name</code> 才被分配内存空间，当方法执行完后，<code v-pre>name</code> 将自动销毁释放空间；</li>
<li><strong>实参</strong>：方法调用时传入的实际值，在方法调用前就已经被初始化且在方法调用时被传入；</li>
</ol>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">func</span><span class="token punctuation">(</span><span class="token class-name">String</span> name<span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">valueCrossTest</span><span class="token punctuation">(</span><span class="token keyword">int</span> age<span class="token punctuation">,</span><span class="token keyword">float</span> weight<span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token class-name">PersonCrossTest</span><span class="token punctuation">(</span><span class="token class-name">Person</span> person<span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token class-name">PersonCrossTest</span><span class="token punctuation">(</span><span class="token class-name">Person</span> person<span class="token punctuation">)</span><span class="token punctuation">{</span>
    ...
    <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"方法内重新赋值后的name："</span> <span class="token operator">+</span> person<span class="token punctuation">.</span><span class="token function">getName</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="面向对象" tabindex="-1"><a class="header-anchor" href="#面向对象" aria-hidden="true">#</a> 面向对象</h2>
<h3 id="面向对象-vs-面向过程" tabindex="-1"><a class="header-anchor" href="#面向对象-vs-面向过程" aria-hidden="true">#</a> 面向对象 vs 面向过程</h3>
<p>推荐阅读：<a href="https://www.zhihu.com/question/27468564/answer/757537214" target="_blank" rel="noopener noreferrer">https://www.zhihu.com/question/27468564/answer/757537214<ExternalLinkIcon/></a></p>
<ul>
...
<h3 id="封装、继承、多态" tabindex="-1"><a class="header-anchor" href="#封装、继承、多态" aria-hidden="true">#</a> 封装、继承、多态</h3>
<ol>
...
<h3 id="成员变量-vs-局部变量-vs-静态变量" tabindex="-1"><a class="header-anchor" href="#成员变量-vs-局部变量-vs-静态变量" aria-hidden="true">#</a> 成员变量 vs 局部变量 vs 静态变量</h3>
<table>
<thead>
<tr>
<th>不同</th>
...
<tr>
<td>成员变量</td>
<td>1、 属于类<br />2、能被访问控制符、<code v-pre>static、final</code> 等修饰</td>
<td>堆</td>
<td>与对象共存亡</td>
<td>有，基本数据类型为对应默认值，而对象统一为 <code v-pre>null</code></td>
<td>对象调用</td>
<td>实例变量</td>
</tr>
<tr>
<td>局部变量</td>
<td>1、属于方法（方法中的变量或参数）<br />2、不能被访问控制符及 <code v-pre>static</code> 修饰，但可以被 <code v-pre>final</code> 修饰</td>
<td>栈</td>
<td>与方法共存亡</td>
<td>无，必须定义赋值后使用</td>
<td></td>
...
<tr>
<td>静态变量</td>
<td>1、属于类<br />2、被 <code v-pre>static</code> 修饰，被所有类对象共用</td>
<td>方法区</td>
<td>与类共存亡</td>
<td>同成员变量初始化值</td>
<td>类名调用（推荐）、对象调用</td>
...
<h3 id="构造方法的特点" tabindex="-1"><a class="header-anchor" href="#构造方法的特点" aria-hidden="true">#</a> 构造方法的特点</h3>
<ol>
<li>方法名与类名同名；</li>
<li>无返回值，但不能用 <code v-pre>void</code> 关键字声明；</li>
<li>生成类对象时自动执行，无需显式调用；</li>
</ol>
<h3 id="抽象类-接口" tabindex="-1"><a class="header-anchor" href="#抽象类-接口" aria-hidden="true">#</a> 抽象类 &amp; 接口</h3>
<ol>
<li><strong>接口</strong></li>
</ol>
<ul>
<li>
<p>接口中所有方法默认是 <code v-pre>public</code>，而且不能有实现（Java 8 之前，Java 8 开始可以有默认实现）；</p>
</li>
<li>
<p>接口中所有变量均为 <code v-pre>static、final</code>，不能有其他变量；</p>
</li>
<li>
<p>一个类可以实现多个接口（通过 <code v-pre>implements</code> 关键字），而且接口自身可以通过 <code v-pre>extends</code> 来扩展多个接口；</p>
</li>
<li>
...
<li>一个类只能实现一个抽象类；</li>
<li>抽象方法可以被 <code v-pre>public、protected、default</code> 修饰，但不能用 <code v-pre>private</code>，否则不能被重写；</li>
<li>抽象是对类的抽象，是一种模板设计；</li>
</ul>
<h3 id="object-类中常见方法" tabindex="-1"><a class="header-anchor" href="#object-类中常见方法" aria-hidden="true">#</a> Object 类中常见方法</h3>
<table>
<thead>
<tr>
<th>方法</th>
...
<tbody>
<tr>
<td><code v-pre>public final native Class&lt;?&gt; getClass()</code></td>
<td>用于返回当前运行时对象的 Class 对象，使用了final 关键字修饰，故不允许子类重写</td>
</tr>
<tr>
<td><code v-pre>public native int hashCode()</code></td>
<td>用于返回对象的哈希码，主要使用在哈希表中，比如 JDK 中的 HashMap</td>
</tr>
<tr>
<td><code v-pre>public boolean equals(Object obj)</code></td>
<td>用于比较 2 个对象的内存地址是否相等，String 类对该方法进行了重写用户比较字符串的值是否相等</td>
</tr>
<tr>
<td><code v-pre>protected native Object clone() throws CloneNotSupportedException</code></td>
...
<tr>
<td><code v-pre>public final native void notify()</code></td>
<td>不能重写。唤醒一个在此对象监视器上等待的线程(监视器相当于就是锁的概念)。如果有多个线程在等待只会任意唤醒一个</td>
</tr>
<tr>
<td><code v-pre>public final native void notifyAll()</code></td>
<td>不能重写。跟notify一样，唯一的区别就是会唤醒在此对象监视器上等待的所有线程，而不是一个线程</td>
</tr>
<tr>
<td><code v-pre>public final native void wait(long timeout) throws InterruptedException</code></td>
<td>不能重写。暂停线程的执行注意：sleep方法没有释放锁，而wait方法释放了锁 。timeout是等待时间，调用该方法后当前线程进入睡眠状态，知道如下时间发生：<br />1. 其他线程调用该对象的 <code v-pre>notify()/notifyAll()</code> 方法；<br />2. 时间间隔到了；<br />3. 其他线程调用了 <code v-pre>interrupt()</code> 中断该线程；</td>
</tr>
<tr>
<td><code v-pre>public final void wait(long timeout, int nanos) throws InterruptedException</code></td>
<td>多了nanos参数，这个参数表示额外时间（以毫微秒为单位，范围是 0-999999）。 所以超时的时间还需要加上 nanos 毫秒</td>
</tr>
<tr>
<td><code v-pre>public final void wait() throws InterruptedException</code></td>
<td>跟之前的 2 个 wait 方法一样，只不过该方法一直等待，没有超时时间这个概念</td>
</tr>
<tr>
<td><code v-pre>protected void finalize() throws Throwable { }</code></td>
<td>实例被垃圾回收器回收的时候触发的操作</td>
</tr>
</tbody>
</table>
<h3 id="hashcode-equals" tabindex="-1"><a class="header-anchor" href="#hashcode-equals" aria-hidden="true">#</a> hashCode &amp; equals</h3>
<p>推荐阅读：<a href="https://juejin.im/post/5a4379d4f265da432003874c" target="_blank" rel="noopener noreferrer">https://juejin.im/post/5a4379d4f265da432003874c<ExternalLinkIcon/></a></p>
<h4 id="equals" tabindex="-1"><a class="header-anchor" href="#equals" aria-hidden="true">#</a> equals</h4>
<ol>
<li>重写 <code v-pre>equals()</code> 方法的准则：</li>
</ol>
<table>
<thead>
<tr>
...
<td>对任意非空引用值 <code v-pre>x</code>，<code v-pre>x.equals(x)</code> 应该返回 <code v-pre>true</code></td>
</tr>
<tr>
<td><strong>对称性</strong></td>
<td>对于任何非空引用值 <code v-pre>x </code>和 <code v-pre>y</code>，当 <code v-pre>y.equals(x)</code> 返回 <code v-pre>true </code>时，<code v-pre>x.equals(y)</code>  也应返回 <code v-pre>true</code></td>
</tr>
<tr>
<td><strong>传递性</strong></td>
<td>对于任何非空引用值<code v-pre> x、y</code> 和 <code v-pre>z</code>，如果 <code v-pre>x.equals(y)</code> 返回 <code v-pre>true</code>， 并且 <code v-pre>y.equals(z)</code> 返回 <code v-pre>true</code>，那么 <code v-pre>x.equals(z) </code> 也应返回 <code v-pre>true</code></td>
</tr>
<tr>
<td><strong>一致性</strong></td>
<td>对于任何非空引用值 <code v-pre>x </code> 和  <code v-pre>y</code>，多次调用 <code v-pre>x.equals(y)</code> 始终返回 <code v-pre>true</code> 或始终返回 <code v-pre>false</code>， 前提是对象上 <code v-pre>equals </code>比较中所用的信息没有被修改</td>
</tr>
<tr>
<td><strong>非空性</strong></td>
<td>对于任何非空引用值 <code v-pre>x</code>，<code v-pre>x.equals(null)</code> 都应返回 <code v-pre>false</code></td>
</tr>
</tbody>
</table>
<h4 id="hashcode" tabindex="-1"><a class="header-anchor" href="#hashcode" aria-hidden="true">#</a> hashCode</h4>
<p><code v-pre>hashCode</code> 用于返回对象 <code v-pre>hash</code> 值，主要是为了加快查找的快捷性，因为 <code v-pre>hashCode()</code> 是 <code v-pre>Object</code> 类中的方法，所以所有 Java 类均有 <code v-pre>hashCode()</code>，在 <code v-pre>HashTable</code> 和 <code v-pre>HashMap</code> 这类的散列结构中，均是通过 <code v-pre>hashCode()</code> 来查找在散列表中位置，通过 <code v-pre>hashCode</code> 能够较快的茶道小内存块。</p>
<h4 id="为什么重写-equals-必须重写-hashcode" tabindex="-1"><a class="header-anchor" href="#为什么重写-equals-必须重写-hashcode" aria-hidden="true">#</a> 为什么重写 <code v-pre>equals()</code> 必须重写 <code v-pre>hashCode()</code></h4>
<ol>
<li>若两个对象相等，则 <code v-pre>hashCode()</code> 一定也相同，因为 <code v-pre>equals()</code> 是绝对可靠的；</li>
<li>两个对象相等，则两个对象分别调用 <code v-pre>equals()</code> 方法也返回 <code v-pre>true</code>；</li>
<li>两个对象有相同的 <code v-pre>hashCode()</code>，他们不一定相等，因为 <code v-pre>hashCode()</code> 不是绝对可靠的；</li>
<li>如果重写了 <code v-pre>equals()</code>，但保留 <code v-pre>hashCode()</code> 的实现不变，则可能出现两者相等，但 <code v-pre>hashCode</code> 却不一样；</li>
<li>因此，一旦重写了 <code v-pre>equals()</code> 方法，则必须重写 <code v-pre>hashCode()</code>，<code v-pre>hashCode() </code> 的默认行为是对堆上的对象产生独特值。如果没有重写 <code v-pre>hashCode()</code>，则该 <code v-pre>class</code> 的两个对象无论如何都不会相等（即使这两个对象指向相同的数据）。</li>
</ol>
...
<h4 id="需要序列化的场景" tabindex="-1"><a class="header-anchor" href="#需要序列化的场景" aria-hidden="true">#</a> 需要序列化的场景</h4>
<p>当 Java 对象需要在网络上传输或者持久化存储到文件中时，我们就需要对象进行序列化；</p>
<h4 id="如何实现序列化" tabindex="-1"><a class="header-anchor" href="#如何实现序列化" aria-hidden="true">#</a> 如何实现序列化</h4>
<p>要实现序列化，只需要让类实现 <code v-pre>Serializable</code> 接口即可，此时就标注该类对象能够被序列化；</p>
<p>针对类中某些数据不想序列化时，可以使用 <code v-pre>transient</code> 关键字来实现，例如：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 通过关键字 transient 修饰，表明不参与序列化</span>
<span class="token keyword">transient</span> <span class="token keyword">private</span> <span class="token class-name">String</span> telephone<span class="token punctuation">;</span>
